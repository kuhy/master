...
development model (Chapter~\ref{sec:org537eaa7}).
After that, the various factors that influence the pull request acceptance are
mentioned (Chapter~\ref{sec:org338743b}). The next part of the thesis
...
of individual pull requests (Chapter~\ref{sec:org652911e}). Then, the used
statistical methods are introduced (Chapter~\ref{sec:orgef25b18}). Furthermore,
the retrieved results are evaluated separately for each programming language
(Chapter~\ref{sec:orgb9593a6}). Towards the end, results are compared between
...
(Chapter~\ref{sec:org58648b2}). Moreover, the potential extensions of my work are
proposed.
\chapter{Code quality in pull-based development}
\label{sec:org537eaa7}
...
\end{itemize}
A comparison between my and their findings is at the end of the thesis (Chapter~\ref{sec:org235dca7}).
How the data about code quality were retrieved is in detail discussed in the following chapter.
\chapter{Data mining}
\label{sec:org652911e}
...
in Table~\ref{tab:orgdd0119e}.
\begin{table}[h]
\caption{\label{tab:orgdd0119e}Data retrieved from GitHub}
\centering
\scriptsize
\begin{tabular}{|llcc|}
\hline
...
used is determined by the file extension of the tested file (Table~\ref{tab:orgf8d5f83}).
\begin{table}[htbp]
\caption{\label{tab:orgf8d5f83}Linters supported by \texttt{git-contrast}}
\centering
\scriptsize
\begin{tabular}{|lcll|}
\hline
...
\end{itemize}
The list of selected projects is available in the appendix (Section~\ref{sec:org28c4e93}). Which statistical
methods were used to evaluate data mined from GitHub is discussed in the following chapter.
\chapter{Data analysis}
\label{sec:orgef25b18}
...
\chapter{Evaluation}
\label{sec:orgb9593a6}
This chapter is dedicated to the findings of my research. The first
five subchapters focus on individual programming languages (see Table~\ref{tab:org3ac4a8d}).
...
\begin{table}[htbp]
\caption{\label{tab:org3ac4a8d}Analyzed programming languages}
\centering
\scriptsize
\begin{tabular}{|lcll|}
\hline
...
\hline
\hline
\hyperref[sec:orgf7c601b]{Python} & 20 & 9452 & 73 \%\\
\hyperref[sec:org0c082e1]{Java} & 20 & 8887 & 73 \%\\
\hyperref[sec:org18b6450]{Kotlin} & 20 & 7514 & 80 \%\\
\hyperref[sec:orgdc4f2cc]{Haskell} & 20 & 6949 & 78 \%\\
\hyperref[sec:org5faa53e]{C/C++} & 20 & 8774 & 77 \%\\
\hline
\end{tabular}
\end{table}
\FloatBarrier
\section{Python}
\label{sec:orgf7c601b}
...
chapter (Chapter \ref{sec:org43a1e81}). It is possible that projects selected using different metrics can yield
varying results.
