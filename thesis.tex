...
development model (Chapter~\ref{sec:org7dad204}).
After that, the various factors that influence the pull request acceptance are
mentioned (Chapter~\ref{sec:org3356238}). The next part of the thesis
...
of individual pull requests (Chapter~\ref{sec:org72340fe}). Then, the used
statistical methods are introduced (Chapter~\ref{sec:org1f1854f}). Furthermore,
the retrieved results are evaluated separately for each programming language
(Chapter~\ref{sec:orgdbabf71}). Towards the end, results are compared between
...
(Chapter~\ref{sec:orgb446cf2}). Moreover, the potential extensions of my work are
proposed.
\chapter{Code quality in pull-based development}
\label{sec:org7dad204}
...
in Table~\ref{tab:orgf684976}.
\begin{table}[h]
\caption{\label{tab:orgf684976}Data retrieved from GitHub}
\centering
\scriptsize
\begin{tabular}{|llcc|}
\hline
...
used is determined by the file extension of the tested file (Table~\ref{tab:orgab3e440}).
\begin{table}[htbp]
\caption{\label{tab:orgab3e440}Linters supported by \texttt{git-contrast}}
\centering
\scriptsize
\begin{tabular}{|lcll|}
\hline
...
\chapter{Evaluation}
\label{sec:orgdbabf71}
This chapter is dedicated to the findings from my research. The first
five subchapters focus on individual programming languages (see Table~\ref{tab:org69f9464}).
...
\begin{table}[htbp]
\caption{\label{tab:org69f9464}Analyzed programming languages}
\centering
\scriptsize
\begin{tabular}{|lcll|}
\hline
...
\hline
\hline
\hyperref[sec:org118ab35]{Python} & 20 & 9452 & 73 \%\\
\hyperref[sec:orgd63571b]{Java} & 20 & 8887 & 73 \%\\
\hyperref[sec:orgef19101]{Kotlin} & 20 & 7514 & 80 \%\\
\hyperref[sec:org64b2269]{Haskell} & 20 & 6949 & 78 \%\\
\hyperref[sec:orge3c305b]{C/C++} & 20 & 8774 & 77 \%\\
\hline
\end{tabular}
\end{table}
\FloatBarrier
\section{Python}
\label{sec:org118ab35}
...
error is \texttt{import-error} (24 \% of PRs); however, there will probably be
...
Like in the Python projects, the pull request from the less popular project
were more likely to be accepted than pull requests from more popular projects (see Figure~\ref{fig:java_stars}).
\tikzFigure{results/java/}{stars_and_acceptance}{Stars and pull request acceptance}{fig:java_stars}
\subsection*{Research Question 1}
\label{sec:org55324e6}
As shown in Figure~\ref{fig:java_quality}, only 1366 pull requests (from the total of 8887 pull requests) did not change
...
\subsection*{Research Question 2}
\label{sec:org794c2b9}
\tikzFigure{results/kotlin/}{issue_importance}{Ten most important ktlint issues}{fig:kotlin_importance}
As shown in Figure~\ref{fig:kotlin_importance},the issue with the highest importance average is \texttt{dot-spacing}.
...
quality issue, I performed the \(\chi^2\) test of independence (between presence of some issue and PR acceptance)
on Kotlin dataset (see Figure~\ref{fig:kotlin_ct}).
...
\subsection*{Research Question 1}
\label{sec:org750ac25}
As mention before, 6949 pull requests were analyzed. Interestingly, in over sixty
percent of pull requests, no change in the code quality was detected (see Figure~\ref{fig:haskell_quality}).
Moreover, the \emph{HLint} is able to recognize a large number of different issues (321 issue
...
and warnings (Figure~\ref{fig:haskell_types}). The error that was introduced in the largest number of pull requests is
...
\subsection*{Research Question 2}
\label{sec:orgc279c21}
As can be seen in Figure~\ref{fig:haskell_importance}, the most important Haskell issue is the suggestion \texttt{Use-if}.
...
\subsection*{Research Question 2}
\label{sec:org383d628}
As shown in Figure~\ref{fig:c_cpp_importance},classification algorithms rank as the most important issue the
...
\subsection*{Research Question 3}
\label{sec:org270cafe}
As can be seen in Figure~\ref{fig:c_cpp_pca},
...
However, the caution is in order because the code quality for each language is
...
chapter (Chapter \ref{sec:org99e24fb}). It is possible that projects selected using different metrics can yield
varying results.
