...
development model (Chapter~\ref{sec:org70efdb9}).
After that, the various factors that influence the pull request acceptance are
mentioned (Chapter~\ref{sec:org038b675}). The next part of the thesis
...
of individual pull requests (Chapter~\ref{sec:orgab4829c}). Then, the used
statistical methods are introduced (Chapter~\ref{sec:org97d4ff7}). Furthermore,
the retrieved results are evaluated separately for each programming language
(Chapter~\ref{sec:org6e23e80}). Towards the end, results are compared between
...
(Chapter~\ref{sec:org241f830}). Moreover, the potential extensions of my work are
proposed.
\chapter{Code quality in pull-based development}
\label{sec:org70efdb9}
...
\end{figure}
Information about the pull requests was retrieved using the \texttt{pr\_quality.py}
...
in Table~\ref{tab:orgd2a2e55}.
\begin{table}[h]
\caption{\label{tab:orgd2a2e55}Data retrieved from GitHub}
\centering
\scriptsize
\begin{tabular}{|llcc|}
\hline
...
used is determined by the file extension of the tested file (Table~\ref{tab:org11ba5d5}).
\begin{table}[htbp]
\caption{\label{tab:org11ba5d5}Linters supported by \texttt{git-contrast}}
\centering
\scriptsize
\begin{tabular}{|lcll|}
\hline
...
\chapter{Evaluation}
\label{sec:org6e23e80}
This chapter is dedicated to the findings of my research. The first
five subchapters focus on individual programming languages (see Table~\ref{tab:org036e981}).
...
\begin{table}[htbp]
\caption{\label{tab:org036e981}Analyzed programming languages}
\centering
\scriptsize
\begin{tabular}{|lcll|}
\hline
...
\hline
\hline
\hyperref[sec:orgfecfe7b]{Python} & 20 & 9452 & 73 \%\\
\hyperref[sec:org30573bd]{Java} & 20 & 8887 & 73 \%\\
\hyperref[sec:orgd641418]{Kotlin} & 20 & 7514 & 80 \%\\
\hyperref[sec:org409e40a]{Haskell} & 20 & 6949 & 78 \%\\
\hyperref[sec:orge9dd903]{C/C++} & 20 & 8774 & 77 \%\\
\hline
\end{tabular}
\end{table}
\FloatBarrier
\section{Python}
\label{sec:orgfecfe7b}
...
chapter (Chapter \ref{sec:org8b171a3}). It is possible that projects selected using different metrics can yield
varying results.
