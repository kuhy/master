...
*** Are there some particular issues/code smells that affect the pull request acceptance?
    ...
    - LogisticRegression :: Despite its name, logistic regression is a linear model used for classification. It uses so
      called /logistic function/ that turns the inputs (code quality issues)
      into the probability of dependent variable (PR acceptance) being 1 (PR is
      accepted).
      - https://en.wikipedia.org/wiki/Logistic_regression#cite_note-1
      - https://scikit-learn.org/stable/modules/linear_model.html#logistic-regression
    - DecisionTrees :: This algorithm constructs the tree where leafs represents the different classes (PR accepted/rejected)
      and inner nodes represents so called /split criterion/ --- the condition
      (or predicate) on single/multiple attributes (code quality issues).
      The /split criterion/ defines to which subtree given data (pull
      request) belong to.
      - https://www.amazon.com/Data-Classification-Algorithms-Applications-Knowledge/dp/1466586745
    - Bagging :: The Bagging algorithm is trying to predict the data class (PR being rejected/accepted)
      using multiple different classifiers. It is using bootstrapping[fn::random sampling with replacement]
      to construct the different data set for each
    ...
    - RandomForest :: This classifier leverages the bagging method in order to create the forest of
      ...
      the RandomForest is using only subset of features (code quality issues) to generate the decision tree
      (this ensures the low correlation between the trees).
      - https://www.ibm.com/cloud/learn/random-forest
    - ExtraTrees :: ExtraTrees is a classifier similar to RandomForest.
      The main difference is that ExtraTrees algorithm generates /split
      criterions/ using randomization.  Another key difference is that
      ExtraTrees is using whole original sample for each tree (instead of
      bootstrapping).
      - https://quantdare.com/what-is-the-difference-between-extra-trees-and-random-forest/
    - AdaBoost :: The AdaBoost is another algorithm that is using multiple weak classifiers (usually DecisionTrees with only one
      /split criterion/) to predict the final result. It begins by fitting a
      classifier on the original dataset.  The each subsequent classifier is
      improved using the results from the previous one (incorrectly classified
      pull requests have the higher chance to be selected in the next
      classifier).
    ...
    - GradientBoost :: The GradientBoost algorithm is similar to the AdaBoost. It is also
      ...
      trains the classifiers using the residual errors of predecessors. Furthermore, the GradientBoost is
      using larger trees than AdaBoost.
    ...
    This mechanism is resource-intense (requires lot of computational power), but is usually more reliable
    then the classic importance mechanisms.

    To evaluate the model accuracy, the dataset was split into five parts
    (5-fold cross-validation).  Each model was then trained five times ---
    distinct dataset was used for training and for validation.  Several metrics
    (precision, recall, AUROC, F-measure\dots) were used to evaluate the
    reliability of each model. Afterwards, the average metrics over all folds
    were computed.
...
*** Is there a relationship between the source code quality and the pull request acceptance?
    ...
    The impact of presence of some code issue in the PR on the PR acceptance was
    determined using the chi-square test. In order to perform this test, dataset
    was transformed to /contingency table/.  This table ($2 \times 2$) contained
    the number of accepted/rejected PRs with/without a code quality issue.
    After that, the chi-square test of independence was performed on the
    ...
    was also computed. The values of Cramer's V ranges between 0 (no
    ...
    acceptance, then it can be a large threat to validity of the independence
    ...
    Moreover, the chi-square test was performed independently for each issue category
    to understand if there are some issue categories that has a higher influence on
    the quality.

    The test was also computed for each project separately. Unluckily,
    there are some projects that contains insufficient number of pull requests.
    According to Cochran[[cite:cochran]], all expected counts should be 10 or greater.
    Therefore, the tests were evaluated only for some projects.
    ...
    determine if the code quality have some impact on the PR acceptance.
*** Does code quality influence the time it takes to close a pull request?
    ...
    Following regressors were used:
    - LinearRegression :: Linear regression is commonly used type of predictive models.
    ...
    - ElasticNet :: ElasticNet is an extension of linear regression. It is adding $L_1$ (lasso regression)
      ...
      /loss function/[fn::function that is minimized during the regression] as a penalty.
      - https://machinelearningcompass.com/machine_learning_models/elastic_net_regression/
    - Some of the already discussed methods used for classification, were used also for regression.
...
*** Is code quality impact higher in projects that are using some particular programming language?
    The *RQ3* discuss the impact of code quality on individual
    programming languages. The findings from the *RQ3* for each
    language are compared in the *RQ5*. This comparison is complicated
    task, because each language has different characteristics and
    different linter was used to measure the code quality.

    The results from chi-square tests were compared to identify
    possible difference between the languages (in terms of code
    ...
    were compared between the languages (using the metrics from regressors).
* Evaluation
...
** Python
   ...
   Almost identical results were obtained when the chi-square test was performed separately for each issue category.
...
** Kotlin
   ...
   quality issue, I performed the chi-square test on Kotlin dataset. The $p < \num{2.2e-16}$ and
   ...
   the chi-square test. There are four projects where the presence of some issue
...
** C/C++
   ...
   Based on the chi-square test, the presence of an issue in the PR has a small negative impact
...
** Programming languages and code quality impact
   ...
   On top of that, the results from the chi-square test can also be compared.
   ...
   primary language, the chi-square test indicates that the presence of issue and PR
   ...
   In all of the languages, the code quality impact is small (based on the chi-square tests
...
** Threats to validity
   ...
   modified a huge number of files were ignored.
