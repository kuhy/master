...
** Python
   ...
   The average importance of the =syntax-error= is only 0.4 %.  The syntax error
   ...
   When the fixed issues were not considered (only introduced issues), the list
   ...
   =protected-access= and =unused-import=. The =syntax-error= is considered
...
** Java
   ...
   \caption{PMD issues and \% of PRs which fixed/introduced them}
   \end{figure}
   #+END_EXPORT
** Kotlin
   #+BEGIN_EXPORT latex
