...
* Data analysis
  ...
  *RQ_2*:
  ...
  - correlation matrix
  - reference the model reliability (from the appendix)
...
** Python
   ...
   higher then expected. For chi-square test, $p < \num{2.2e-16}$ and therefore, the hypothesis that presence of
   some issue and PR acceptance are independent is rejected on significance level $\alpha = 0.05$. However,
   the Cramer's $\phi_c \approx 0.092$ therefore the association is weak.
   ...
   primary language. For this test, $p < \num{5.548e-10}$ and therefore the
   ...
   relation between presence of same issue and PR acceptance is small.

   *TODO:* Adjust p-value when doing multiple tests.

   *TODO:* evaluate all chi-square tests (projects, issue types)
